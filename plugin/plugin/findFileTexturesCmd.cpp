...
MStatus findFileTextures::doIt( const MArgList& args )
{
	...
	for ( MItSelectionList iter( list ); !iter.isDone(); iter.next() ) {
		...
		if ( !status ) {
			...
			// In here, we wanted to get to the nodes that instObjGroups.objectGroups connected
			// to since we know that the shadingEngine connects to the instObjGroup.objectGroups
			// attribute.For example polyShape.f[0] is connected to one shadingSG.
			//
			MDagPath dagPath ;
        ...
        //
        // instObjGroups is a multi attribute.  
        //
		for(unsigned int j=0;j<iogPlug.numConnectedElements();j++){
			iogPlug.elementByLogicalIndex(j).connectedTo( iogConnections, false, true, &status );
